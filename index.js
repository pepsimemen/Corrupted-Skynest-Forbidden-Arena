...
module.exports = function Tera_Guide(mod) {
	...
	let hooks              = [],
		...
		bossWord           = null;  // 愤怒/恐惧
	// 控制命令
	mod.command.add(["cguide", "csguide"], (arg) => {
		if (!arg) {
			Enabled = !Enabled;
			mod.command.message("Corrupted Skynest Guide" + (Enabled ? "ON" : "OFF"));
		} else {
			switch (arg) {
				case "st":
				case "stream":
				...
				case "if":
					...
					mod.command.message("Stream Mode: "   + SendToStream);
	...
	function sActionStage(event) {
		...
		if (whichmode==3020 && event.templateId==2200) {
			...
			// 三连击 开始技能
			if ([121, 122].includes(skillid)) {
				bossBuff = skillid;
				return;
			}
			// 三连击 结束技能
			if ([120, 123].includes(skillid)) {
		...
		if (whichmode==3027 && event.templateId==1000) {
			if (event.stage!=0 || !(bossSkillID = FA_BOSS.find(obj => obj.id==skillid))) return;
			// 紫/红 鉴定预测
